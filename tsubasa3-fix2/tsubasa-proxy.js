...
class Tsubasa {
  ...
  initHeaders() {
    const firstUserId = JSON.parse(
      decodeURIComponent(this.data[0].split("user=")[1].split("&")[0])
    ).id;
    return {
      ...
      "X-Masterhash":
        "fcd309c672b6ede14f2416cca64caa8ceae4040470f67e83a6964aeb68594bbc",
    };
  }
  ...
  loadConfig() {
    ...
    try {
      ...
      config.enableTapUpgrades =
        config.enableTapUpgrades !== undefined
          ? config.enableTapUpgrades
          : true;
      config.enableEnergyUpgrades =
        config.enableEnergyUpgrades !== undefined
          ? config.enableEnergyUpgrades
          : true;
  ...
  async countdown(seconds) {
    ...
    for (let i = seconds; i >= 0; i--) {
      ...
      process.stdout.write(
        `${`[${folderName}] [*] Chờ ${i} giây để tiếp tục...`}`
      );
  ...
  async callDailyRewardAPI(initData, axiosInstance) {
    ...
    try {
      ...
      return {
        success: false,
        message: `Lỗi điểm danh hàng ngày: ${error.message}`,
      };
    }
  }
    ...
    do {
      ...
      for (const card of sortedCards) {
        ...
        if (card.end_datetime && currentTime > card.end_datetime) {
          this.log(
            `Thẻ ${card.name} (${card.cardId}) đã hết hạn. Bỏ qua nâng cấp.`,
            "warning"
          );
          continue;
        }
        ...
        if (
          ...
          try {
            ...
            if (levelUpResponse.status === 200) {
              ...
              leveledUp = true;
              this.log(
                `Nâng cấp thẻ ${card.name} (${card.cardId}) lên level ${
                  card.level + 1
                }. Cost: ${card.cost}, Balance còn: ${updatedTotalCoins}`
              );
              break;
            }
          } catch (error) {
            if (
              ...
              this.log(
                `Chưa đến thời gian nâng cấp tiếp theo cho thẻ ${card.name} (${card.cardId})`,
                "warning"
              );
              ...
              console.log(error);
              this.log(
                `Lỗi nâng cấp thẻ ${card.name} (${card.cardId}): ${error.message}`,
                "error"
              );
            }
          }
  ...
  async tapAndRecover(initData, axiosInstance) {
    ...
    while (continueProcess) {
      ...
      while (currentEnergy > 0) {
        ...
        totalTaps += tapcount;
        this.log(
          `Tap thành công | Năng lượng còn ${tapResult.energy}/${tapResult.max_energy} | Balance : ${tapResult.total_coins}`,
          "success"
        );
        currentEnergy = 0;
        ...
        if (recoveryResult.energy === maxEnergy) {
          ...
          this.log(
            `Hồi năng lượng thành công | Năng lượng hiện tại: ${currentEnergy}/${maxEnergy}`,
            "success"
          );
        } else {
          this.log(
            `Hồi năng lượng không đủ | Năng lượng hiện tại: ${recoveryResult.energy}/${maxEnergy}`,
            "warning"
          );
          continueProcess = false;
          break;
  ...
  async callTapLevelUpAPI(initData, axiosInstance) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          success: false,
          error: `Lỗi nâng cấp tap: ${response.status}`,
        };
      }
    } catch (error) {
  ...
  async callEnergyLevelUpAPI(initData, axiosInstance) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          success: false,
          error: `Lỗi nâng cấp energy: ${response.status}`,
        };
      }
    } catch (error) {
    ...
    if (this.config.enableTapUpgrades) {
      while (
        ...
        if (tapUpgradeResult.success) {
          ...
          this.log(
            `Nâng cấp Tap thành công | Level: ${tap_level} | Cost: ${tap_level_up_cost} | Balance: ${total_coins}`,
            "success"
          );
        } else {
    ...
    if (this.config.enableEnergyUpgrades) {
      while (
        ...
        if (energyUpgradeResult.success) {
          ...
          this.log(
            `Nâng cấp Energy thành công | Level: ${energy_level} | Cost: ${energy_level_up_cost} | Balance: ${total_coins}`,
            "success"
          );
        } else {
    ...
    while (true) {
      for (let i = 0; i < this.data.length; i++) {
        ...
        try {
          ...
          this.log(
            `Lỗi kiểm tra IP của proxy cho tài khoản ${i + 1}: ${
              error.message
            }`,
            "warning"
          );
        ...
        try {
          ...
          if (startResult.success) {
            if (startResult.total_coins !== undefined) {
              ...
              this.log(
                `Năng lượng: ${startResult.energy}/${startResult.max_energy}`
              );
              ...
              for (const task of startResult.tasks) {
                ...
                if (executeResult) {
                  ...
                  if (achievementResult.success) {
                    this.log(
                      `Làm nhiệm vụ ${achievementResult.title} thành công | phần thưởng ${achievementResult.reward}`,
                      "success"
                    );
                  }
                }
            ...
            this.log(
              `Đã nâng cấp hết các thẻ đủ điều kiện | Balance: ${updatedTotalCoins}`,
              "success"
            );
