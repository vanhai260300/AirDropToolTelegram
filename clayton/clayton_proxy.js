...
class Clayton {
  ...
  async log(msg, type = "info") {
    ...
    const folderName = "Clayton";
    let logMessage = "";

    switch (type) {
      case "success":
        console.log(`[${timestamp}] [${folderName}] [*] ${msg}`.green);
        break;
      case "custom":
        console.log(`[${timestamp}] [${folderName}] [*] ${msg}`.magenta);
        break;
      case "error":
        console.log(`[${timestamp}] [${folderName}] [!] ${msg}`.red);
        break;
      case "warning":
        console.log(`[${timestamp}] [${folderName}] [*] ${msg}`.yellow);
        break;
      default:
        console.log(`[${timestamp}] [${folderName}] [*] ${msg}`.blue);
    }
  ...
  async login() {
    return this.makeRequest(
      "https://tonclayton.fun/api/user/authorization",
      "post"
    );
  }
  ...
  async dailyClaim() {
    return this.makeRequest(
      "https://tonclayton.fun/api/user/daily-claim",
      "post"
    );
  }
  ...
  async completePartnerTask(taskId) {
    return this.makeRequest(
      "https://tonclayton.fun/api/tasks/complete",
      "post",
      { task_id: taskId }
    );
  }
  ...
  async rewardPartnerTask(taskId) {
    return this.makeRequest("https://tonclayton.fun/api/tasks/claim", "post", {
      task_id: taskId,
    });
  }
  ...
  async handlePartnerTasks() {
    ...
    while (fetchAttempts < maxAttempts) {
      ...
      if (tasksResult.success) {
        ...
        for (const task of uncompletedTasks) {
          ...
          while (taskAttempts < maxAttempts) {
            ...
            if (completeResult.success) {
              ...
              if (rewardResult.success) {
                this.log(
                  `Làm nhiệm vụ ${task.task.title} thành công. Nhận được ${task.task.reward_tokens} CL`,
                  "success"
                );
                break;
              }
          ...
          if (taskAttempts === maxAttempts) {
            this.log(
              `Không thể hoàn thành nhiệm vụ ${task.task.title} sau ${maxAttempts} lần thử. Bỏ qua nhiệm vụ này.`,
              "error"
            );
          }
    ...
    if (fetchAttempts === maxAttempts) {
      this.log(
        `Không thể lấy danh sách nhiệm vụ đối tác sau ${maxAttempts} lần thử. Bỏ qua xử lý nhiệm vụ đối tác.`,
        "error"
      );
    }
  }
  ...
  async handleDailyTasks() {
          ...
          while (taskAttempts < maxAttempts) {
            ...
            if (completeResult.success) {
              ...
              if (claimResult.success) {
                this.log(
                  `Làm nhiệm vụ ${task.task.title} thành công. Nhận được ${claimResult.data.reward_tokens} CL`,
                  "success"
                );
                this.log(
                  `Tổng CL: ${claimResult.data.total_tokens} | Số lượt chơi game: ${claimResult.data.game_attempts}`,
                  "info"
                );
                break;
              } else {
                this.log(
                  `Không thể nhận phần thưởng cho nhiệm vụ ${
                    task.task.title
    ...
    if (fetchAttempts === maxAttempts) {
      this.log(
        `Không thể lấy danh sách nhiệm vụ hàng ngày sau ${maxAttempts} lần thử. Bỏ qua xử lý nhiệm vụ hàng ngày.`,
        "error"
      );
    }
  }
  ...
  async play2048() {
    const startGameResult = await this.makeRequest(
      "https://tonclayton.fun/api/game/start",
      "post"
    );
    if (
    ...
    for (const milestone of allMilestones) {
      ...
      const saveGameResult = await this.makeRequest(
        "https://tonclayton.fun/api/game/save-tile",
        "post",
        { maxTile: milestone }
      );
      if (
    ...
    const endGameResult = await this.makeRequest(
      "https://tonclayton.fun/api/game/over",
      "post",
      { multiplier: 1 }
    );
    if (endGameResult.success) {
      ...
      this.log(
        `Trò chơi 2048 đã kết thúc thành công. Nhận ${reward.earn} CL và ${reward.xp_earned} XP`,
        "success"
      );
    } else {
      this.log(
        `Lỗi kết thúc trò chơi 2048: ${
          endGameResult.error || "Lỗi không xác định"
        }`,
        "error"
      );
    }
  ...
  async playStack() {
    const startGameResult = await this.makeRequest(
      "https://tonclayton.fun/api/stack/start-game",
      "post"
    );
    ...
    while (Date.now() < gameEndTime && currentScoreIndex < scores.length) {
      ...
      const updateResult = await this.makeRequest(
        "https://tonclayton.fun/api/stack/update-game",
        "post",
        { score }
      );
      if (updateResult.success) {
        ...
        this.log(
          `Lỗi cập nhật điểm Stack: ${
            updateResult.error || "Lỗi không xác định"
          }`,
          "error"
        );
      }
    ...
    const endGameResult = await this.makeRequest(
      "https://tonclayton.fun/api/stack/end-game",
      "post",
    ...
    if (endGameResult.success) {
      const reward = endGameResult.data;
      this.log(
        `Trò chơi Stack đã kết thúc thành công. Nhận ${reward.earn} CL và ${reward.xp_earned} XP`,
        "success"
      );
    } else {
      this.log(
        `Lỗi kết thúc trò chơi Stack: ${
          endGameResult.error || "Lỗi không xác định"
        }`,
        "error"
      );
  ...
  async handleDefaultTasks() {
    ...
    while (attempts < maxAttempts) {
      attempts++;
      tasksResult = await this.makeRequest(
    ...
    if (!tasksResult.success) {
      this.log(
        `Không thể lấy danh sách nhiệm vụ mặc định sau ${maxAttempts} lần thử. Bỏ qua xử lý nhiệm vụ mặc định.`,
        "error"
      );
      return;
    }

    const incompleteTasks = tasksResult.data.filter(
      (task) => !task.is_completed && task.task_id !== 9
    );
    ...
    for (const task of incompleteTasks) {
      ...
      const claimResult = await this.makeRequest(
        "https://tonclayton.fun/api/tasks/claim",
        "post",
        { task_id: task.task_id }
      );
      ...
      if (claimResult.success) {
        ...
        this.log(
          `Làm nhiệm vụ ${task.task.title} thành công. Phần thưởng ${reward.reward_tokens} CL | Balance: ${reward.total_tokens}`,
          "success"
        );
      } else {
        this.log(
          `Không thể nhận phần thưởng cho nhiệm vụ ${task.task.title}: ${
            claimResult.error || "Lỗi không xác định"
          }`,
          "error"
        );
  ...
  async handleSuperTasks() {
    ...
    while (attempts < maxAttempts) {
      attempts++;
      SuperTasks = await this.makeRequest(
        "https://tonclayton.fun/api/tasks/super-tasks",
        "get"
      );
    ...
    if (!SuperTasks.success) {
      this.log(
        `Không thể lấy danh sách nhiệm vụ cao cấp sau ${maxAttempts} lần thử. Bỏ qua xử lý nhiệm vụ cao cấp.`,
        "error"
      );
      return;
    }

    const incompleteTasks = SuperTasks.data.filter(
  ...
  async processAccount() {
    ...
    if (!loginSuccess) {
      this.log(
        "Đăng nhập không thành công sau 3 lần thử. Bỏ qua tài khoản.",
        "error"
      );
      return;
    }
    ...
    this.log(
      `CL: ${userInfo.tokens} CL | ${userInfo.daily_attempts} Ticket`,
      "info"
    );
