...
class Matchain {
  constructor() {
    ...
    this.autogame = true;
    this.proxies = path.join(__dirname, "proxy.txt");
  }
  ...
  log(msg, level = "info") {
    ...
    const folderName = "Matchain";
    console.log(`[${folderName}] [*] ${msg}`[levels[level]]);
  }
  ...
  async login(data, proxy) {
    ...
    if (taskStatus) {
      if (taskStatus.dailyNeedsPurchase) {
        try {
          const boosterResult = await this.buyBooster(
            token,
            this.userid,
            proxy
          );
    ...
    while (gameCount > 0) {
      ...
      const point = Math.floor(Math.random() * (150 - 130 + 1)) + 130;
  ...
  async getTaskList(uid, proxy) {
    ...
    const matchainEcosystem = Array.isArray(data["Matchain Ecosystem"])
      ? data["Matchain Ecosystem"]
      : [];
    const allTasks = [...data.Tasks, ...extraTasks, ...matchainEcosystem];
