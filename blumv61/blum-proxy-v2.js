...
class GameBot {
  ...
  async log(msg, type = "info") {
    ...
    let logMessage = "";
    const folderName = "Blum";

    switch (type) {
      case "success":
        logMessage = `[${timestamp}] [${folderName}] [*] ${msg}`.green;
        break;
      case "error":
        logMessage = `[${timestamp}] [${folderName}] [!] ${msg}`.red;
        break;
      case "warning":
        logMessage = `[${timestamp}] [${folderName}] [*] ${msg}`.yellow;
        break;
      default:
        logMessage = `[${timestamp}] [${folderName}] [*] ${msg}`.blue;
    }
  ...
  async Countdown(seconds) {
    ...
    for (let i = Math.floor(seconds); i >= 0; i--) {
      ...
      process.stdout.write(
        `${`[${folderName}] [Tài khoản ${this.accountIndex + 1}]`.padEnd(
          15
        )} [*] Chờ ${i} giây để tiếp tục...`
      );
  ...
  async runAccount() {
    try {
        ...
        for (const task of taskFilter) {
          switch (task.status) {
            ...
            case "READY_FOR_VERIFY":
              ...
              if (
                claimResultAfterVerify &&
                claimResultAfterVerify.status === "FINISHED"
              ) {
                await this.log(
            ...
            default:
              ...
              if (
                claimResultDefault &&
                claimResultDefault.status === "FINISHED"
              ) {
                await this.log(
                  `Làm nhiệm vụ ${task.title.yellow}${
                    `... trạng thái: thành công!`.green
      ...
      await this.log(
        `Lỗi không xác định khi xử lý tài khoản: ${error.message}`,
        "error"
      );
      return Duration.fromMillis(0);
    }
        ...
        workerPromises.push(
          new Promise((resolve) => {
            worker.on("message", (message) => {
              if (message.error) {
                errors.push(
                  `Tài khoản ${message.accountIndex}: ${message.error}`
                );
              } else {
            ...
            worker.on("error", (error) => {
              errors.push(
                `Lỗi worker cho tài khoản ${currentIndex}: ${error.message}`
              );
              resolve();
            });
            worker.on("exit", (code) => {
              if (code !== 0) {
                errors.push(
                  `Worker cho tài khoản ${currentIndex} thoát với mã: ${code}`
                );
              }
              resolve();
            });
