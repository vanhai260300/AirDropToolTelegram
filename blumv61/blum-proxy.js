  ...
  async Countdown(seconds) {
    for (let i = Math.floor(seconds); i >= 0; i--) {
      ...
      process.stdout.write(
        `${`[Tài khoản ${this.accountIndex + 1}]`.padEnd(
          15
      ...
      for (const task of notStartedTasks) {
        ...
        if (startResult) {
          ...
          if (claimResult && claimResult.status === "FINISHED") {
            ...
            await this.log(
              `Không thể nhận phần thưởng cho nhiệm vụ: ${task.title}`,
              "error"
            );
          }
        } else {
        ...
        while (playAttempts < maxAttempts) {
          try {
            ...
            if (playResult) {
              ...
              const randomNumber =
                Math.floor(Math.random() * (200 - 165 + 1)) + 165;
            ...
            playAttempts++;
            await this.log(
              `Không thể chơi game lần thứ ${j + 1}, lần thử ${playAttempts}: ${
                error.message
              }`,
              "warning"
            );
