...
class VooiDC:
    ...
    def log(self, msg, type='info'):
        ...
        folder_name = "Vooi"
        if type == 'success':
            print(f"[{timestamp}] [{folder_name}] [*] {Fore.GREEN}{msg}")
        elif type == 'custom':
            print(f"[{timestamp}] [{folder_name}] [*] {Fore.MAGENTA}{msg}")
        elif type == 'error':
            print(f"[{timestamp}] [{folder_name}] [!] {Fore.RED}{msg}")
        elif type == 'warning':
            print(f"[{timestamp}] [{folder_name}] [*] {Fore.YELLOW}{msg}")
        else:
            print(f"[{timestamp}] [{folder_name}] [*] {Fore.BLUE}{msg}")

    def countdown(self, seconds):
        folder_name = "Vooi"
        for i in range(seconds, -1, -1):
            print(f"\r[{folder_name}] [*] Chờ {i} giây để tiếp tục...", end="", flush=True)
            time.sleep(1)
        print()
